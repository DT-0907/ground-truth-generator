"""
Flask web application for video track correction.
...
for d in [VIDEOS_DIR, TRACKS_DIR, CORRECTIONS_DIR, EXPORTS_DIR]:
    d.mkdir(parents=True, exist_ok=True)

# NAS config file path
NAS_CONFIG_FILE = BASE_DIR / "config" / "nas.json"

# Mode state
_active_mode = 'local'  # 'local' or 'nas'
_nas_mount_point = None
_session_map = {}  # session_id -> absolute video Path

# Store local paths so we can restore them
...
def build_session_id(video_path, root):
    """Generate URL-safe session_id from video's path relative to root."""
...
def get_video_path(session_id):
    """Look up actual video file path for a session_id."""
    if session_id in _session_map:
        return _session_map[session_id]
    # In NAS mode, populate map lazily if empty
    if _active_mode == 'nas' and not _session_map:
        get_videos()
        if session_id in _session_map:
            return _session_map[session_id]
    # Fallback: local flat lookup
...
processing_jobs = {}  # session_id -> {status, progress, error, started_at}
export_jobs = {}      # session_id -> {status, progress, error, started_at, output_dir}
processing_lock = threading.Lock()


def get_sessions():
    """Get all available sessions (videos with track files)."""
    sessions = []
    ...
    for track_file in sorted(track_files):
        ...
        # Load track data to get video info
        with open(track_file, 'r') as f:
            data = json.load(f)

        # Find video file (uses _session_map for NAS, flat lookup for local)
...
def review(session_id):
    """Review page for a session."""
    track_file = TRACKS_DIR / f"{session_id}.json"
    if not track_file.exists():
        return "Session not found", 404

    # Load corrections first if they exist, otherwise original tracks
    correction_file = CORRECTIONS_DIR / f"{session_id}.json"
...
def api_get_tracks(session_id):
    ...
    # First try corrections, then original tracks
    correction_file = CORRECTIONS_DIR / f"{session_id}.json"
    track_file = TRACKS_DIR / f"{session_id}.json"
...
def api_merge_tracks(session_id):
    ...
    try:
        ...
        # Merge frames, deduplicating by frame number (keep higher confidence)
...
def get_videos():
    ...
    if _active_mode == 'nas':
        ...
        candidates = sorted(VIDEOS_DIR.iterdir())
    ...
    for f in candidates:
        ...
        # Check processing status — use session_id as job key
        status = 'unprocessed'
        track_count = 0
...
def api_process_video():
    ...
    # If video_name was passed (legacy), derive session_id from stem
...
def api_video_thumbnail(session_id):
    """Get a thumbnail frame from a video."""
    video_path = get_video_path(session_id)
    if not video_path or not video_path.exists():
        return "Video not found", 404
    ...
    # Grab a frame from ~10% in (skip any black intro frames)
...
def export_labeled_images(session_id, sample_rate=1):
    """Export frames with bounding boxes drawn on them.

    Args:
        session_id: session identifier (used for video lookup and output naming)
        sample_rate: export every Nth frame that has detections (1 = all)
    """
    # Find video file
    video_path = get_video_path(session_id)
    if not video_path or not video_path.exists():
    ...
    # Load corrections first, then tracks
    correction_file = CORRECTIONS_DIR / f"{session_id}.json"
    track_file = TRACKS_DIR / f"{session_id}.json"
...
def api_export_video():
    """Start exporting labeled images for a video in the background."""
    session_id = request.json.get('session_id') or request.json.get('video_name', '')
    if not request.json.get('session_id') and session_id:
...
# --- NAS mount/unmount + config persistence ---

def load_nas_config():
    """Load saved NAS config (decodes password)."""
...
def save_nas_config(config):
    """Save NAS config (encodes password with base64)."""
    safe = dict(config)
...
def mount_nas(config):
    """Mount SMB share. Returns (success, message, mount_point)."""
...
def api_nas_status():
    """Get current NAS mode and connection info."""
    result = {
        'mode': _active_mode,
...
def api_nas_config():
    """Get saved NAS config (without password)."""
    config = load_nas_config()
    if config:
        safe = dict(config)
...
def api_nas_connect():
    """Mount NAS and switch to NAS mode."""
    data = request.json
...
def api_nas_disconnect():
    """Switch back to local mode and unmount NAS."""
...
def check_nas_on_startup():
    """If NAS was previously configured and still mounted, reconnect."""
    config = load_nas_config()
    if config:
        mount_point = Path(config.get('mount_point') or '/tmp/cctv_nas_mount')
...
check_nas_on_startup()


if __name__ == '__main__':
    print("=" * 50)
    print("Vehicle Track Correction UI")
    print("=" * 50)
