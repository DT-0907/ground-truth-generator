"""
Sessions tab — shows all processed sessions with stats cards and session list.

Layout: stats cards row at top + scrollable session list below.
"""
...
class SessionsTab(QWidget):
    """Sessions tab content — stat cards + scrollable session list."""
    ...
    def _setup_ui(self):
        ...
        self.stat_review = self._make_stat_card("Need Review", "0")
    ...
    def _make_stat_card(self, label_text, value_text):
        ...
        lbl.setStyleSheet(f"color: #8899aa; font-size: 11px; letter-spacing: 1px; border: none;")
    ...
    def refresh(self):
        ...
        if not sessions:
            empty = QLabel("No sessions found. Process some videos first.")
    ...
    def _make_session_card(self, session):
        ...
        detail_text = f"{session.get('track_count', 0)} tracks"
        if session.get("processed_at") and session["processed_at"] != "Unknown":
            detail_text += f"  |  {session['processed_at']}"
